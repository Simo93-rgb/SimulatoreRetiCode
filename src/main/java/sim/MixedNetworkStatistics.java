...
public class MixedNetworkStatistics {
    ...
    public void updateAreas(double currentTime) {
        ...
        if (serverBusyQ1) {
            if (serverBusyQ1Open) areaBusyQ1Open   += dt;
            else                  areaBusyQ1Closed += dt;
        }
        areaQueueQ1  += queueLengthQ1 * dt;
        areaSystemQ1 += (queueLengthQ1 + (serverBusyQ1 ? 1 : 0)) * dt;

        // Q2
        if (serverBusyQ2) areaBusyQ2 += dt;
    ...
    public double getThroughputQ1Closed(double T) { return completionsQ1Closed / T; }

    /** Throughput Q1 classe aperta. */
    public double getThroughputQ1Open(double T)   { return completionsQ1Open   / T; }
    ...
    public double getUtilizationQ1Closed(double T) { return areaBusyQ1Closed / T; }

    /** Utilizzo Q1 imputabile alla classe aperta. */
    public double getUtilizationQ1Open(double T)   { return areaBusyQ1Open   / T; }

    /** E[Nq1] — lunghezza media coda Q1. */
    public double getMeanQueueLengthQ1(double T) { return areaQueueQ1 / T; }

    /** E[N1] — numero medio nel centro Q1. */
    public double getMeanSystemSizeQ1(double T)  { return areaSystemQ1 / T; }
    ...
    public double getThroughputQ2(double T) { return completionsQ2 / T; }

    /** Utilizzo Q2. */
    public double getUtilizationQ2(double T) { return areaBusyQ2 / T; }

    /** E[Nq2]. */
    public double getMeanQueueLengthQ2(double T) { return areaQueueQ2 / T; }

    /** E[N2]. */
    public double getMeanSystemSizeQ2(double T)  { return areaSystemQ2 / T; }
    ...
    /**
     * Throughput sistema visto da Q0 = throughput classe chiusa su Q1.
     * In steady state equivale al throughput di Q2.
     */
    public double getSystemThroughput(double T) { return getThroughputQ1Closed(T); }

    /** E[T_sys] sistema centrale classe chiusa = E[T1_closed] + E[T2]. */
    public double getMeanResponseTimeSystemClosed() {
        return getMeanResponseTimeQ1Closed() + getMeanResponseTimeQ2();
    }

    // ---- Getters raw ----
    public long getCompletionsQ1Closed() { return completionsQ1Closed; }
    public long getCompletionsQ1Open()   { return completionsQ1Open; }
